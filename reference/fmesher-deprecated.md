...
Some of these functions still attempt to do their job, but will be
...
- `fm_nonconvex_hull_inla()`: **\[deprecated\]** Legacy method for
  `INLA::inla.nonconvex.hull()`. Use
  ...
  compatibility with code expecting `fm_segm` output) or
  `format = "sf"`.
...
version `0.4.0.9002`, the `fm_nonconvex_hull_inla()` function is
deprecated, in favour of the more configurable update version of
[`fm_nonconvex_hull()`](https://inlabru-org.github.io/fmesher/reference/fm_nonconvex_hull.md).
