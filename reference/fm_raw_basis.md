...
without necessarily matching the default function space of the given
mesh object.
...
- type:
  ...
  `b.spline` (default) for B-spline basis functions, `sph.harm` for
  spherical harmonics (available only for meshes on the sphere)

- n:

  For B-splines, the number of basis functions in each direction (for 1d
  meshes `n` must be a scalar, and for planar 2d meshes a 2-vector). For
  spherical harmonics, `n` is the maximal harmonic order.
...
- free.clamped:
  ...
  If `TRUE` and `boundary` is `"free"`, the boundary basis functions are
  clamped to 0/1 at the interval boundary by repeating the boundary
  knots. See
  [`fm_mesh_1d()`](https://inlabru-org.github.io/fmesher/reference/fm_mesh_1d.md)
  for more information.
