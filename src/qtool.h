...
public:
   ...
   */
  SMatrix const &Q() const { return Q_; };
  SCholesky const &LLt() {
  ...
  void invalidate_partial_inverse() { has_partial_inverse_ = false; };
  ...
  /** Factorisation requires pattern analysis, and is prerequisite for partial
   * inverse
   *
   ...
   *
   * Basic recursion:
   * For \f$i=n-1,\dots,0\f$, and \f$j=n-1,\dots,i\f$, if \f$L_{ji}\f$ is not
   * known to be zero, let \f{align*}{ S_{ji} = S_{ij} &= \left(\mathbb{I}(i=j)
   * / L_{ii} - \sum_{k=i+1}^{n-1} L_{ki} S_{kj} \right) / L_{ii}
   * \\									\
   *   &= \begin{cases}
   *         \left(1 / L_{ii} - \sum_{k=i+1}^{n-1} L_{ki} S_{kj} \right) /
   * L_{ii}
   *         & \text{if $i=j$,} \\
   ...
   *           S[j,i] = S[i,j] = (I(i==j)/L[i,i] - sum_{k=i+1}^{n-1} L[k,i]
   * S[k,j] ) / L[i,i]
   *
  ...
  void partialInversion() {
    ...
    for (int64_t i = n - 1; i >= 0; --i) {
      ...
      for (typename SMatrix::ReverseInnerIterator LcolI0(L, i); LcolI0;
           --LcolI0) {
        //      int64_t j = LcolI0.row();
        //      FMLOG_("Processing i = " << i << ", j = " << LcolI0.row() <<
        //      std::endl);
  ...
  void solveL(const Eigen::MatrixBase<Rhs> &b,
    ...
    Eigen::MatrixBase<Lhs> &result_ =
        const_cast<Eigen::MatrixBase<Lhs> &>(result);
  ...
  void solve(const Eigen::SparseMatrixBase<Rhs> &b,
    ...
    Eigen::SparseMatrixBase<Lhs> &result_ =
        const_cast<Eigen::SparseMatrixBase<Lhs> &>(result);
    result_.derived().resize(Q_.rows(), b.cols());
    result_ = LLt_.solve(b);
  }
};
