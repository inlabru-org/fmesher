...
public:
  ...
  bool operator<(const MCQdv &tb) const {
    return ((value_ < tb.value_) || ((value_ == tb.value_) && (d_ < tb.d_)));
  }
};
...
public:
  MCQ(MeshC *MC, bool only_quality)
      : MC_(MC), darts_(), darts_quality_(), only_quality_(only_quality){};
...
public:
  ...
  void insert(const Dart &d) { MCQ::insert(Dart(*d.M(), d.t())); };
  void erase(const Dart &d) { MCQ::erase(Dart(*d.M(), d.t())); };
  bool found(const Dart &d) const { return MCQ::found(Dart(*d.M(), d.t())); };
  bool foundQ(const Dart &d) const { return MCQ::foundQ(Dart(*d.M(), d.t())); };
  double quality(const Dart &d) const {
    return MCQ::quality(Dart(*d.M(), d.t()));
  };
  Dart quality() const { return MCQ::quality(); };
...
public:
  MCQsegm(MeshC *MC)
      : MCQ(MC, false), meta_(), encroached_limit_(10 * MESH_EPSILON){};
...
public:
  enum State {
    State_noT = 0, /*!< No triangulation present */
...
private:
  ...
  // double* big_limits_; /*!< Big triangle limits. Unused; MCQtri has
  // quality_limits_ */
  ...
  */
  Dart CDTInsertSegment(const int v0, const int v1, triangleSetT &triangles,
                        const bool is_boundary, const constrMetaT &meta);
  /*!
