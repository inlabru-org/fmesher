library(fmesher)
fm_block_log_shift_old <- function(
  block = NULL,
  log_weights = NULL,
  n_block = NULL
) {
...
fm_block_log_shift_new <- function(
  block = NULL,
  log_weights = NULL,
  n_block = NULL
) {
...
print(bench::mark(
  old = fm_block_log_shift_old(
    block = block,
    log_weights = log_weights,
    n_block = n_block
  ),
  new = fm_block_log_shift_new(
    block = block,
    log_weights = log_weights,
    n_block = n_block
  )
...
ggplot(times) +
  geom_line(aes(x = n_block, y = as.numeric(median), col = method)) +
  facet_wrap(~N) +
  scale_y_log10()
