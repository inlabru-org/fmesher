...
for (k in seq_len(nrow(vec))) {
  ...
  edges <- list(
    mesh[[k]]$loc[mesh[[k]]$graph$tv[, 2], ] -
      mesh[[k]]$loc[mesh[[k]]$graph$tv[, 1], ],
    mesh[[k]]$loc[mesh[[k]]$graph$tv[, 3], ] -
      mesh[[k]]$loc[mesh[[k]]$graph$tv[, 2], ],
    mesh[[k]]$loc[mesh[[k]]$graph$tv[, 1], ] -
      mesh[[k]]$loc[mesh[[k]]$graph$tv[, 3], ]
  )
      ...
      pmax(
        ...
        c(
          -rowSums(edges[[1]] * edges[[2]]) /
            rowSums(edges[[1]]^2)^0.5 /
            rowSums(edges[[2]]^2)^0.5,
          -rowSums(edges[[2]] * edges[[3]]) /
            rowSums(edges[[2]]^2)^0.5 /
            rowSums(edges[[3]]^2)^0.5,
...
for (k in seq_len(nrow(vec) - 1) + 1) {
  ...
  title(paste0(
    "min.angle = ",
    round(vec$min.angle[k], 3),
    " < ",
    round(res$min.angle[k], 3),
    ", ",
    "max.edge = ",
    round(vec$max.edge[k], 3),
    " > ",
    round(res$max.edge[k], 3)
  ))
  Sys.sleep(0.1)
}
