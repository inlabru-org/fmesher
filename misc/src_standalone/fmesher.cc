...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  matrices.io(
      ((args_info.io_arg == io_arg_ba) || (args_info.io_arg == io_arg_bb)),
      ((args_info.io_arg == io_arg_ab) || (args_info.io_arg == io_arg_bb)));
  ...
  if ((iS0.rows() > 0) && (iS0.cols() < 2)) {
    ...
    for (size_t i = 1; i < M.nV(); i++) {
      ...
      if (issphere) {
        issphere =
            (std::fabs(M.S(i).length() / radius - 1.0) < sphere_tolerance);
      }
    }
    if (!isflat) {
      if (issphere) {
    ...
    */
    ...
    if (useX11) {
      if (issphere) {
        ...
        M.useX11(true, useX11text, 500, 500, x11_zoom[0], x11_zoom[1],
                 x11_zoom[2], x11_zoom[3]);
      } else {
    ...
    if (args_info.smorg_given > 0) {
      ...
      if (!isflat && !issphere) {
        ...
        if (args_info.rcdt_given) {
          /* Calculate the RCDT: */
          MC.RCDT(rcdt_min_angle, rcdt_big_limit_auto_default, Quality0.raw(),
                  Quality0.rows(), rcdt_max_n0, rcdt_max_n1);
          FMLOG(MC << endl);
        }
