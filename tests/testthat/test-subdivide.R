test_that("fmesher_subdivide works", {
  mesh <- fm_rcdt_2d_inla(rbind(c(0, 0), c(1, 0), c(0, 1)),
                          rbind(c(0, 1, 2)) + 1L)#,
#                          refine=list(max.edge = 0.1))
  ...
  sub0 <- fmesher_subdivide(mesh$loc,
                            mesh$graph$tv - 1L,
  ...
  # 5, 4
  # 0, 3, 1
  sub1.tv <- rbind(c(0, 3, 5),
                   c(3, 1, 4),
                   c(4, 5, 3),
                   c(5, 4, 2))
  sub1.loc <- rbind(mesh$loc,
                    cbind(rbind(c(0.5, 0), c(0.5, 0.5), c(0, 0.5)), 0))
  sub1 <- fmesher_subdivide(mesh$loc,
                            mesh$graph$tv - 1L,
                            mesh$segm$bnd$idx - 1L,
                            mesh$segm$int$idx - 1L,
                            1L,
  ...
  sub2.tv <- rbind(c(0, 3, 8),
                   ...
                   c(6, 7, 9),
                   c(7, 6, 2))
  sub2.loc <- rbind(mesh$loc,
                    cbind(rbind(c(1/3, 0), c(2/3, 0),
                                c(2/3, 1/3), c(1/3, 2/3),
                                c(0, 2/3), c(0, 1/3),
                                c(1/3, 1/3)), 0))
  sub2 <- fmesher_subdivide(mesh$loc,
                            mesh$graph$tv - 1L,
                            mesh$segm$bnd$idx - 1L,
                            mesh$segm$int$idx - 1L,
                            2L,
...
test_that("fm_subdivide works for S2 meshes", {
  ...
  expect_equal(mean(rowSums(mesh_s2_1$loc^2)),
               mean(rowSums(mesh_s2$loc^2)))
  expect_equal(mean(rowSums(mesh_s2_1_1$loc^2)),
               mean(rowSums(mesh_s2$loc^2)))
  expect_equal(mean(rowSums(mesh_s2_3$loc^2)),
               mean(rowSums(mesh_s2$loc^2)))
